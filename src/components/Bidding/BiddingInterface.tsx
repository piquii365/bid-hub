...
export default function BiddingInterface({ property, biddingRoom }: BiddingInterfaceProps) {
  ...
  const handleSubmitBid = async (e: React.FormEvent) => {
    ...
    if (amount <= property.currentPrice) {
      setError(`Bid must be higher than current price of $${property.currentPrice.toLocaleString()}`);
      setIsSubmitting(false);
      return;
    }

    if (property.bidType === 'live' && amount < suggestedBid) {
      setError(`Minimum bid increment is $${minBidIncrement}. Suggested bid: $${suggestedBid.toLocaleString()}`);
      setIsSubmitting(false);
      return;
    }
    ...
    <div className={`space-y-6 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className={`p-3 rounded-lg ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-50'}`}>
            <div className="flex items-center space-x-2 mb-1">
              <Timer className={`h-4 w-4 ${biddingRoom.timeRemaining < 300 ? 'text-red-500' : 'text-blue-500'}`} />
              <span className="text-sm font-medium">Time Left</span>
            </div>
            <div className={`text-lg font-bold ${biddingRoom.timeRemaining < 300 ? 'text-red-500' : ''}`}>
      ...
      <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        <div className="flex items-center justify-between mb-2">
          ...
          {property.currentPrice > property.startingPrice && property.bidType !== 'fixed' && (
            <div className="flex items-center space-x-1 text-green-500">
              <ArrowUp className="h-4 w-4" />
              <span className="text-sm">+${(property.currentPrice - property.startingPrice).toLocaleString()}</span>
            </div>
          )}
        </div>
        <div className="text-3xl font-bold">${property.currentPrice.toLocaleString()}</div>
        {property.bidType !== 'fixed' && property.startingPrice !== property.currentPrice && (
      ...
      {property.bidType === 'live' && property.status === 'active' && (
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[suggestedBid, suggestedBid + 500, suggestedBid + 1000].map((amount) => (
              <button
                key={amount}
      ...
      {property.status === 'active' && (
        <form onSubmit={handleSubmitBid} className="space-y-4">
          <div>
            <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
              {property.bidType === 'fixed' ? 'Purchase Amount' : 'Your Bid Amount'}
            </label>
            <div className="relative">
              <DollarSign className={`absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`} />
              <input
                type="number"
                value={bidAmount}
                onChange={(e) => setBidAmount(e.target.value)}
                placeholder={property.bidType === 'fixed' ? property.currentPrice.toString() : suggestedBid.toString()}
                ...
                min={property.bidType === 'fixed' ? property.currentPrice : property.currentPrice + 1}
                step="1"
                required
              />
            </div>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span>Placing {property.bidType === 'fixed' ? 'Order' : 'Bid'}...</span>
              </>
            ) : (
              <>
                {config.icon}
      ...
      {property.status !== 'active' && (
        <div className={`p-4 rounded-lg text-center ${
          ...
          <h3 className={`font-semibold mb-2 ${
            property.status === 'sold' ? 'text-green-800 dark:text-green-200' : theme === 'dark' ? 'text-gray-200' : 'text-gray-800'
          }`}>
            {property.status === 'sold' ? 'Auction Completed' : 'Auction Ended'}
          </h3>
          <p className={`text-sm ${
            property.status === 'sold' ? 'text-green-600 dark:text-green-300' : theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
          }`}>
            {property.status === 'sold' 
